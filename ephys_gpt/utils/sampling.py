...
def sample_from_logits(
    ...
    """
    Samples from the logits using either the top-p or top-k sampling.

    If neither top_p nor top_k are specified, argmax sampling is used,
    where the token with the highest logit is selected.
    ...
    if top_p is not None and top_k is not None:
        raise ValueError("'top_p' and 'top_k' cannot be both specified.")
